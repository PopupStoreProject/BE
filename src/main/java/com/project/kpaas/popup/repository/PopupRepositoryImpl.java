...
public class PopupRepositoryImpl {
    ...
    public List<Popupstore> findAllByGps(Point gps, double lat, double lon, double rad) {
        ...
        return new ArrayList<>(result.select(popupstore)
                .from(popupstore)
                .where(
                        Expressions.numberTemplate(Double.class, "ST_Distance_Sphere({0}, POINT({1}, {2}))", gps , centerPoint.getY(), centerPoint.getY())
                                .loe(radius)
                )
